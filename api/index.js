...
// --- IMPORTANTE: CARGAR DOTENV ANTES QUE NADA ---
dotenv.config(); 
...
// Ruta de Validación de Cupón (CORREGIDA)
app.post('/api/coupons/validate', authenticateToken_Permissive, async (req, res) => {
    const { couponCode } = req.body;
    const user = req.user; // Puede ser null si no está logueado
    ...
    try {
        ...
        // 2. Validaciones Generales (Activo y Límite Global)
        if (!coupon.is_active) {
            return res.status(400).json({ error: 'Este cupón ha expirado o no está activo.' });
        }
        
        if (coupon.current_uses >= coupon.max_uses) {
            return res.status(400).json({ error: 'Este cupón ha alcanzado su límite de usos global.' });
        }

        // 3. Validaciones de Usuario (Si el usuario está logueado)
        if (user) {
            // A. Validar Tier (Rango)
            if (coupon.required_tier && user.tier !== coupon.required_tier) {
                return res.status(403).json({ error: `Este cupón es exclusivo para usuarios ${coupon.required_tier}.` });
            }

            // B. Validar Asignación Directa (Si es exclusivo para un ID)
            if (coupon.assigned_user_id && String(coupon.assigned_user_id) !== String(user.id)) {
                 return res.status(403).json({ error: 'Este cupón no te pertenece.' });
            }

            // --- C. NUEVA VALIDACIÓN: ¿EL USUARIO YA USÓ ESTE CUPÓN? ---
            // Buscamos en la tabla orders si existe alguna orden de este usuario con este cupón
            const usageCheck = await pool.query(
                'SELECT id FROM orders WHERE user_id = $1 AND coupon_id = $2 LIMIT 1',
                [user.id, coupon.id]
            );

            if (usageCheck.rows.length > 0) {
                return res.status(400).json({ error: 'Ya has canjeado este cupón en una compra anterior.' });
            }
            // ------------------------------------------------------------
        } else {
            // Si el cupón requiere validaciones de usuario y no hay sesión:
            if (coupon.required_tier || coupon.assigned_user_id) {
                 return res.status(403).json({ error: `Debes iniciar sesión para usar este cupón.` });
            }
        }
...
// Eliminar Cupón (CORREGIDO: Soft Delete / Desactivación)
app.delete('/api/coupons/:id', authenticateToken, isAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        // EN LUGAR DE BORRAR FÍSICAMENTE, LO DESACTIVAMOS.
        // Esto evita el error de llave foránea si el cupón ya tiene pedidos asociados.
        ...
        res.json({ message: 'Cupón eliminado (archivado) con éxito. Ya no podrá ser utilizado.' });

    } catch (err) {
        console.error('Error archiving coupon:', err.stack);
        res.status(500).json({ error: 'Error interno del servidor.' });
    }
});
...
// NUEVA RUTA: Obtener historial de recargas (Aprobadas/Rechazadas)
app.get('/api/admin/recharges/history', authenticateToken, isAdmin, async (req, res) => {
    try {
        const query = `
...
// --- RUTAS DE CONFIGURACIÓN DE RECARGAS (ADMIN Y PÚBLICO) ---

// 1. Obtener Configuración Pública (Para el Usuario: Tasa, WhatsApp y Métodos activos)
app.get('/api/config/recharge', async (req, res) => {
    try {
        // Obtener ajustes globales
        const settingsRes = await pool.query("SELECT key, value FROM system_settings WHERE key IN ('exchange_rate', 'whatsapp_number')");
        const settings = {};
        settingsRes.rows.forEach(row => { settings[row.key] = row.value; });

        // Obtener métodos de pago activos
        const methodsRes = await pool.query("SELECT * FROM payment_methods WHERE is_active = TRUE ORDER BY id ASC");

        res.json({
            exchange_rate: parseFloat(settings.exchange_rate || 3.55),
...
// 2. ADMIN: Actualizar Ajustes Globales (Tasa y WhatsApp)
app.put('/api/admin/config/settings', authenticateToken, isAdmin, async (req, res) => {
    const { exchange_rate, whatsapp_number } = req.body;
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        await client.query("INSERT INTO system_settings (key, value) VALUES ('exchange_rate', $1) ON CONFLICT (key) DO UPDATE SET value = $1", [exchange_rate]);
        await client.query("INSERT INTO system_settings (key, value) VALUES ('whatsapp_number', $1) ON CONFLICT (key) DO UPDATE SET value = $1", [whatsapp_number]);
        await client.query('COMMIT');
...
app.post('/api/admin/payment-methods', authenticateToken, isAdmin, upload.single('image'), async (req, res) => {
    ...
    try {
        const query = `
            INSERT INTO payment_methods (name, account_name, image_url, currency, is_active)
            VALUES ($1, $2, $3, $4, TRUE) RETURNING *;
        `;
        const result = await pool.query(query, [name, account_name, image_url, currency || 'SOL']);
        res.status(201).json(result.rows[0]);
    } catch (err) {
        console.error(err);
...
// 4. ADMIN: Eliminar Método de Pago
...
// 1. NUEVAS RUTAS AUXILIARES (Para llenar los filtros del Admin)
// --- EN TU ARCHIVO index.js ---

// 1. RUTAS DE LISTAS (Para los filtros desplegables)
app.get('/api/admin/lists/providers', authenticateToken, isAdmin, async (req, res) => {
    try {
        const result = await pool.query("SELECT id, username FROM users WHERE role = 'proveedor' ORDER BY username");
        res.json(result.rows);
    } catch (err) { res.status(500).json({ error: 'Error providers.' }); }
});

app.get('/api/admin/lists/products', authenticateToken, isAdmin, async (req, res) => {
    try {
        const result = await pool.query("SELECT id, name FROM products ORDER BY name");
        res.json(result.rows);
    } catch (err) { res.status(500).json({ error: 'Error products.' }); }
});

// 2. RUTA PRINCIPAL DE REPORTES (CORREGIDA PARA TU SCHEMA)
app.get('/api/admin/reports/orders', authenticateToken, isAdmin, async (req, res) => {
    const { search, start_date, end_date, category_id, status, provider_id, product_id } = req.query;
    const params = [];
    let whereClauses = [];
    ...
    if (search) {
        params.push(`%${search}%`);
        // Buscamos en ID de orden, Usuario cliente o Nombre de Producto
    ...
    if (provider_id) {
        ...
        whereClauses.push(`oi.provider_id = $${params.length}`);
    }
    if (product_id) {
        params.push(product_id);
        whereClauses.push(`oi.product_id = $${params.length}`);
    }

    const filterClause = whereClauses.length > 0 ? ' WHERE ' + whereClauses.join(' AND ') : '';

    // --- QUERY SQL ---
    // Usamos STRING_AGG para concatenar los productos y proveedores de una misma orden
    const query = `
        SELECT
            o.id,
            o.created_at,
            ...
            -- Agregados: Cantidad de items y Subtotal
            COUNT(oi.id) AS item_count,
            COALESCE(SUM(oi.price_per_unit_usd * oi.quantity), 0) AS subtotal_bruto,

            -- Agregados: Nombres concatenados (Productos y Proveedores)
            STRING_AGG(DISTINCT p.name, ', ') AS product_names,
            STRING_AGG(DISTINCT u_prov.username, ', ') AS provider_names

        FROM orders o
        LEFT JOIN users u_user ON o.user_id = u_user.id
        LEFT JOIN order_items oi ON o.id = oi.order_id
        LEFT JOIN products p ON oi.product_id = p.id
        LEFT JOIN users u_prov ON oi.provider_id = u_prov.id
        LEFT JOIN coupons c ON o.coupon_id = c.id
        
        ${filterClause}
        
        GROUP BY o.id, u_user.username, u_user.email, c.code
        ORDER BY o.created_at DESC
        LIMIT 50;
    `;
    ...
    try {
        ...
        console.error('Error en reporte:', err); // Mira la consola si falla
        res.status(500).json({ error: 'Error interno obteniendo pedidos.' });
    }
});
