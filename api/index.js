  ...
  try {
    const query = `
      SELECT
        ...
        p.description, p.terms_conditions,  -- <--- AGREGADO
...
// 2. ACTUALIZAR LA RUTA DE REPORTES (/api/admin/reports/orders)
// Busca la ruta existente y reemplázala o actualiza la lógica de los "if":
...
app.get('/api/provider/transfer-history', authenticateToken, isProvider, async (req, res) => {
    try {
        ...
        const query = `
            ...
            FROM transactions 
            WHERE user_id = $1 AND type = 'envio_saldo' OR type = 'recarga_recibida' -- (Opcional: ajustar filtros según necesites)
            AND user_id = $1
            ORDER BY created_at DESC
            LIMIT 50
        `;
